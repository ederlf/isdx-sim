...
def bitstring_2_mac(vmac_bitstring, ss_instance):
    v = int(vmac_bitstring, 2)
    width = int(ss_instance.VMAC_size / 4)

    vmac_addr = '{num:0{width}x}'.format(num = v, width=width)
    vmac_addr = ':'.join([vmac_addr[i] + vmac_addr[i+1] for i in range(0,width, 2)])
    return vmac_addr
